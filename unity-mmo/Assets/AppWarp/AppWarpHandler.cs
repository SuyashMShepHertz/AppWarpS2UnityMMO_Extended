...
public class AppWarpHandler : MonoBehaviour {
	...
	public void onChat(string sender, string message)
	{
		if(sender != USER)
		{
			...
			if(obj != null)
			{
				...
				else if(int.Parse(msg["type"]) == 3)
				{
					obj.GetComponent<RemoteCharacter>().doPunch();
				}
				else if(int.Parse(msg["type"]) == 4)
				{
					obj.GetComponent<RemoteCharacter>().doJump();
				}
				else if(int.Parse(msg["type"]) == 5)
				{
					GetComponent<Character>().health -= 10;
					obj.GetComponent<RemoteCharacter>().doPunchEffect(transform.position);
	...
	public void sendJump()
	{
		if(WarpClient.GetInstance().GetConnectionState() == WarpConnectionState.CONNECTED)
		{
			string msg = "{name:\""+USER+"\",type:4}";//4 - Jump
			WarpClient.GetInstance().SendChat(msg);
		}
	}
	...
	public void sendHit()
	{
		if(WarpClient.GetInstance().GetConnectionState() == WarpConnectionState.CONNECTED)
		{
			string msg = "{name:\""+USER+"\",type:5}";//5 - Hit
			WarpClient.GetInstance().SendChat(msg);
		}
	}
